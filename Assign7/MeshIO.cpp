...
vec3& getFloats(const string& floats_string) {
	float nums[3];
	std::stringstream ss(floats_string);
	for(int i = 0; i < 3; i++) {
		ss >> nums[i];
	}
...
int3& getDecrementedInts(const string& int_string) {
	std::stringstream ss(int_string);
	vector<int>nums;
	for(int i = 0; ss>>i;) {
		nums.push_back(i-1);
	}
	return int3(nums.at(0), nums.at(1), nums.at(2));
}

bool ReadAsciiObjAlternate(char *filename,
	vector<vec3>&points,
	...
	while(fgets(line, 1000, in)) {
		...
		if(line_str.length() > 0) {
			// extracting types v, vn ('n'), and f
			type = line_str.at(0);
			if(type == 'v' && line_str.at(1)=='n') {
				type = 'n';
			}
			if (type == 'v') {
				temp_points = getFloats(line_str.substr(2));
				points.push_back(temp_points);
			} 
			else if (type=='n') {
				temp_normals = getFloats(line_str.substr(2));
				normals->push_back(temp_normals);
			}
			else if(type=='f') {
				temp_triangle = getDecrementedInts(line_str.substr(2));
				triangles.push_back(temp_triangle);
			}
			// ignoring any other possible character values
		}
